...
describe('scanSkills', () => {
  let home = ''
  before(async () => {
    home = await mkdtemp(join(tmpdir(), 'dsm-test-'))
...
describe('setSkillState', () => {
  ...
  let dir = ''
  before(async () => {
    home = await mkdtemp(join(tmpdir(), 'dsm-state-'))
...
`
  before(async () => {
    dir = await mkdtemp(join(tmpdir(), 'dsm-yaml-'))
...
describe('verify', () => {
  let dir = ''
  before(async () => {
    const home = await mkdtemp(join(tmpdir(), 'dsm-verify-'))
...
describe('findSkills', () => {
  it('names a root-level skill after its repository when the frontmatter cannot', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'dsm-root-'))
  ...
  it('finds skills nested a plugin deep', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'dsm-deep-'))
  ...
  it('finds every skill in a repository, not just the first', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsm-find-'))
