...
var SkillMcpService = class extends TypertRemoteService {
  ...
  async startAppInstall(payload) {
    ...
    void this.appInstaller.install(previewId, (stage2, current, total, detail) => Object.assign(job, { stage: stage2, current, total, detail })).then((result) => {
      this.appJobs.set(jobId, { ...job, state: "done", stage: "complete", current: 1, total: 1, detail: "\u5B89\u88C5\u4E0E\u9A8C\u6536\u5B8C\u6210", result });
      this.invalidate();
    }).catch((cause) => {
      this.appJobs.set(jobId, { ...job, state: "failed", error: cause.message, detail: "\u5B89\u88C5\u5931\u8D25" });
    });
